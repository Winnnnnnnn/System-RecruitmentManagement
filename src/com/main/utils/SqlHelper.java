...
public class SqlHelper {
    ...
    public static void main(String[] args) {
        //测试单例
        AdminBean adminBean = doObjQuery("select * from admin",null,AdminBean.class);
        System.out.println("--------单例测试--------");
        if (adminBean != null) {
            System.out.println("id"+ adminBean.getId());
            System.out.println("用户名"+ adminBean.getName());
            System.out.println("密码"+ adminBean.getPwd());
        } else {
            System.out.println("单例为空");
        }
        System.out.println("--------集合测试--------");
        //测试集合
        List<AdminBean> adminBeanList = doListQuery("select * from admin",null,AdminBean.class);
        if (adminBeanList != null && adminBeanList.size() !=0 ) {
            for (int i = 0; i < adminBeanList.size(); i++) {
                System.out.println("用户名" + (i+1) + adminBeanList.get(i).getName());
                System.out.println("密码" + (i+1) + adminBeanList.get(i).getPwd());
            }
        } else {
